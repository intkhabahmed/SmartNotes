...
class ImageNotesDetailFragment : Fragment() {
    ...
    private fun setupUI() {
        mDetailBinding.run {
            imageNoteView.visibility = View.VISIBLE
            val imageFile = File(mNote?.description!!)
            if (imageFile.exists()) {
                Glide.with(parentActivity!!).load(Uri.fromFile(imageFile)).into(imageNoteView)
            }
            tvNoteTitle.text = mNote!!.noteTitle
            tvDateCreated.text = getFormattedTime(mNote?.dateCreated ?: 0L)
            tvDateModified.text = if (mNote?.dateModified != 0L) getFormattedTime(mNote!!.dateModified) else "-"
            ...
            bannerAdView = AdView(parentActivity, getString(R.string.image_detail_banner_placement_id), AdSize.BANNER_HEIGHT_50)
